...
# General QP solver used by both standard OCS and, later, SQP.
from .qp_solver import highs_active_set_qp
...
def _highs_standard_genetics_current(
    sigma: sparse.spmatrix,
    mu: npt.NDArray[np.float64],
    sires,  # type could be np.ndarray, sets[ints], lists[int], range, etc
    dams,   # type could be np.ndarray, sets[ints], lists[int], range, etc
...
def highs_standard_genetics(
    ...
    """Solve standard OCS using the new general QP solver and verify it.

    This method is now responsible for constructing the OCS-specific sire and
    dam constraints.  The separate ``highs_active_set_qp`` method receives only
    general QP data and has no knowledge of sires, dams, or OCS.

    During development, the new method is called first and the previous
    OCS-specific implementation is called second.  Their solutions and
    objective values are compared as requested by the supervisor.
    """
    ...
    if mu.shape != (dimension,):
        raise ValueError(
            f"mu must have shape ({dimension},), got {mu.shape}."
        )
    ...
    # The general QP variable is x = [w, z].  Standard OCS does not need z,
    # but including it checks that the QP method supports the variable layout
    # required by the robust SQP subproblems.  Its lower bound is zero, its
    # objective coefficient is zero, and it does not occur in the two standard
    # OCS constraints, so HiGHS returns z = 0.
    ...
    # HiGHS uses minimisation.  This represents
    #   min -mu^T w + (lambda/2) w^T sigma w + 0*z.
    ...
    # OCS-specific constraints are formed here, in the caller, rather than in
    # the general QP solver.  The final column is the coefficient of z.
    constraint_matrix: npt.NDArray[np.float64] = np.zeros(
        (2, qp_dimension),
        dtype=np.float64
    )
    constraint_matrix[0, sires] = 1.0
    constraint_matrix[1, dams] = 1.0
    rhs: npt.NDArray[np.float64] = np.full(
        2,
        0.5,
        dtype=np.float64
    )
    ...
    general_model_output = (
        f"{model_output}_general" if model_output else ""
    )
    current_model_output = (
        f"{model_output}_current" if model_output else ""
    )

    # New, problem-independent QP solver.  It returns the complete x=[w,z]
    # solution and the minimisation objective value.
    general_solution, general_minimum = highs_active_set_qp(
        hessian=hessian,
        ...
        upper_bound=upper_x,
        time_limit=time_limit,
        model_output=general_model_output,
        debug=debug,
    ...
    general_w = general_solution[:dimension]
    general_z = float(general_solution[-1])
    general_objective = -general_minimum

    # Previous OCS-specific QP solver, retained temporarily only to check the
    # new implementation against the existing result.
    current_w, current_objective = _highs_standard_genetics_current(
        sigma=sigma,
        mu=mu,
        sires=sires,
        dams=dams,
        ...
        reduced_cost_tol=reduced_cost_tol,
        active_tol=active_tol,
        check_reduced_costs=check_reduced_costs
    )

    solution_difference = float(
        np.max(np.abs(general_w - current_w))
    )
    objective_difference = float(
        abs(general_objective - current_objective)
    )
    comparison_tol = max(1e-7, 10.0 * reduced_cost_tol)

    if debug:
        print("\nComparison of general and current standard QP solvers")
        print(f"  general z = {general_z:.12e}")
        print(
            "  max |w_general - w_current| = "
            f"{solution_difference:.12e}"
        )
        print(
            "  |objective_general - objective_current| = "
            f"{objective_difference:.12e}"
        )

    if abs(general_z) > comparison_tol:
        raise RuntimeError(
            "The additional standard-OCS z variable was expected to be zero, "
            f"but the new QP solver returned z={general_z}."
        )

    if solution_difference > comparison_tol:
        raise RuntimeError(
            "The new general QP solver does not match the current standard "
            "QP solver: max solution difference is "
            f"{solution_difference:.3e}."
        )

    if objective_difference > comparison_tol:
        raise RuntimeError(
            "The new general QP solver does not match the current standard "
            "QP solver: objective difference is "
            f"{objective_difference:.3e}."
        )
